...
def bayesian_recursion(observations,
    ...
    max_num_latents = 999
    table_assignment_priors = torch.zeros((num_obs, max_num_latents), dtype=torch.float32)
    table_assignment_priors[0, 0] = 1.
    table_assignment_posteriors = torch.zeros((num_obs, max_num_latents), dtype=torch.float32)

    table_assignment_posteriors_running_sum = torch.zeros_like(table_assignment_posteriors)

    num_table_posteriors = torch.zeros((num_obs, max_num_latents), dtype=torch.float32)

    cluster_p_params = torch.full(
    ...
    epsilon = 1e-2
    optimizer = torch.optim.SGD(params=[cluster_p_params], lr=1.)
    ...
    for obs_idx, torch_observation in enumerate(torch_observations):
        ...
        cluster_p_params.data[obs_idx, :] = torch_observation
        ...
        for em_idx in range(num_em_steps):
            ...
            cont_bern = torch.distributions.ContinuousBernoulli(
                probs=cluster_p_params[:obs_idx+1])
            ...
            #     2. * torch.arctanh(1. - 2. * cluster_p_params[:obs_idx+1]),
            #     1. - 2. * cluster_p_params[:obs_idx+1])
            # normalizing_const[torch.isnan(normalizing_const)] = 2.
            # likelihoods_per_obs_dim = torch.multiply(
            #     normalizing_const,
            #     torch.multiply(
            #         torch.pow(cluster_p_params[:obs_idx + 1], torch_observation),
            #         torch.pow(1. - cluster_p_params[:obs_idx + 1], 1. - torch_observation)))
            ...
            if obs_idx == 0:
                ...
                assert torch.allclose(torch.sum(num_table_posteriors[obs_idx, :]), 1.)
            ...
            cluster_p_params_update = torch.multiply(
                prefactor[:, None],
                cluster_p_params.grad)
            cluster_p_params.data += em_learning_rate * cluster_p_params_update

            assert torch.all(cluster_p_params[:obs_idx+1] >= epsilon)
            assert torch.all(cluster_p_params[:obs_idx+1] <= 1. - epsilon)
